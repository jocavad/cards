...
<div id="crdDialog" title="<s:message code="crd.CardsTitle"/>">
             <c:if test="${crdPage>1}">
             <input id="e5" name="p" value="<s:message code="Previous"/>" type="button" onclick="$('#crdDialog').load('<c:url value="/cards/list" /> #crdDialog',{crdPage:'${crdPage-1}', ltype:'${ltype}'} );"/>
             </c:if>
             <c:if test="${crdLastPage==0}">
             <input id="e4" name="n" value="<s:message code="Next"/>" type="button" onclick="$('#crdDialog').load('<c:url value="/cards/list" /> #crdDialog',{crdPage:'${crdPage+1}', ltype:'${ltype}'} );"/>
             </c:if>
        <table border="1">
              <tr>
